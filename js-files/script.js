...
function initialize() {
    ...
    // Array in welches die Werte reingeschrieben werden um Überschneidungen
    // duch Multithreading zu vermeiden
    latencyTimeStartArray = new Array();
    downloadTimeStartArray = new Array();

    /*
     * Kurze Messmethode (Lange wird nicht verwendet, da kürze mehr Sinn macht,
     * da sie bei schlechterer Verbindung besser gemessen werden kann und da sie
     * mehr Messpunkte produziert)
     */
    shortMeasure = true;

    // Schaut ob Benutzer eingeloggt ist, wenn nicht wird er zu Login
    // weitergeleitet
    isLogedIn();
...
function geoStart() {
	...
	// Geolocation geht nicht, setze Status auf false, damit nicht gemessen
	// wird
	geolocationStatus = false;

	// Zeige Status
	$("#measureState").html("<font>Geolocation Error!</font>").css(
...
function geoCallback(position) {
	...
	$("#measureState").html("<font>Position unver&auml;ndert!</font>").css(
		"background-color", "#ff0000");
	...
	// Koordinaten sind zu ungenau (konnte keine genaue GPS-Poistion
	// bestimmen)
	$("#measureState").html("<font>GPS-Signal zu schwach!</font>").css(
		"background-color", "#ff0000");
	$(".geoBox").css("border-color", "#ff0000");
	...
	log("origin wird mit den Folgenden Werten zugewiesen: " + originLat
		+ ", " + originLng);
...
function extraGeoCallback(position) {
    ...
    save(downloadRateEnd, latencyEnd, latRoundedGeo, lngRoundedGeo,
	    timeStampGeo, "geo");
...
function geoErrorCallback(error) {
    ...
    // Zeige Status
    $("#measureState").html("<font>Ortsbestimmung fehlerhaft!</font>").css(
	    "background-color", "#ff0000");
    $(".geoBox").css("border-color", "#ff0000");
...
function createHeader() {
    ...
    headerString = "VERSION:speedTracker Handy v1.1"
	    + "\r"
	    + "DATAVERSION:1.1"
	    + "\r"
	    + "MOVEMENTTYPE:"
	    + document.getElementById("movementTypeId").options[document
		    .getElementById("movementTypeId").selectedIndex].value
	    + "\r"
	    + "FILE:"
	    + "http://speedtracks.org/client/downloadFiles/100kb.txt"
	    + "\r"
	    + "FILESIZE:"
	    + "102400"
	    + "\r"
	    ...
	    + "\r"
	    + "NETWORK:"
	    + document.getElementById("networkId").options[document
		    .getElementById("networkId").selectedIndex].value + "\r"
	    + "\r";
...
function initSpeedTracker(p_trackmode) {
    ...
    logFile = logFile + "%0A" + "%0A" + "%0A"
	    + "Starte neue Aufnahme der Logs : " + "%0A";
	...
	// document.getElementById("numberOfSpeed").firstChild.data =
	// numberOfmeasurements;
    }

    $("#measureState").html("<font>Messung wird gestartet..</font>").css(
...
function startIntervalMeasure(measureOption, p_trackmode) {
	...
	if (isAndroidPhone) {
	    ...
	    document.getElementById("hardwareId").value = "Web Applikation";
    }
    ...
    // setze Cookie movementTypeId mit dem Wert aus dem Formular und
    // Verfallsdatum
    setCookie("movementTypeId", $("#movementTypeId option:selected").val(),
	    cookieExpiry);
...
function intervalMeasure(p_trackmode) {
	...
	if (p_trackmode) {
	    $("#measureCount")
		    .html("<font>" + numberOfmeasurements + "</font>");
	} else
	...
	if (shortMeasure) {
	     ...
	     * console.log("geoThreadP = false in 9 sec"); extraGeoMeasure1(); }
	     */
	} else {
...
function printTrack(p_dataUrlString, p_trackmode) {
    ...
    if (p_dataUrlString == "url50")
	datasize = 50; // setze Größe durch Url
    else if (p_dataUrlString == "url100")
	datasize = 100;
    else if (p_dataUrlString == "url150")
	datasize = 150;
    ...
    if (latencyShow == null)
	latencyShow = latencyEnd;

    if (downloadRateEnd >= 0 && latencyEnd >= 0) {
	if (p_trackmode) {
	    save(downloadRateEnd, latencyEnd, originLat, originLng, timeStamp,
		    "track");
...
function run(p_trackmode) { // Trackmode oder Einzelmessung -> Trackmode: true =
    ...
    if (p_trackmode) {
	$("#measureState").html("<font>Messung l&auml;uft ...</font>").css(
		"background-color", "#009FE4");
	...
	if (numberOfmeasurements < minTrackPoints) {
	    ...
	    $("#stopButton")
		    .css(
			    "background",
			    "-webkit-gradient(linear, left top, left bottom, from(#FFAD72), color-stop(0.25, #FF9347), color-stop(0.50, #EC6600), color-stop(0.75, #FF9347), to(#FFAD72))");
	    $("#stopButton")
		    .css(
			    "background",
			    "-moz-linear-gradient(top, #FFAD72 0%, #FF9347 25%, #EC6600 50%, #FF9347 75%, #FFAD72)");
	}
	...
	/*
	 * if(downloadTimeSum != 0) { // Latenzzeit berechnen
	 * getLatencyShort(url100); } else latencyTimeSum = 0;
	 */
    } else {
...
// url50->speedtrack.org/.../50kb.txt
{
	...
	success : function(data) {
	    ...
	    latencyTimeSum = latencyTimeSum
		    + (latencyTimeEnd - latencyTimeStart);
	...
	error : function(jqXHR, textStatus, errorThrown) {
	    // Zeige Status
	    $("#measureState")
		    .html("<font>Latenzermittlung fehlerhaft!</font>").css(
			    "background-color", "#ff0000");
...
function getLatency() {
	...
	xhr.onreadystatechange = function() {
		...
		if (xhr.status == 200) {
		    ...
		    latencyTimeSum = latencyTimeSum
			    + (latencyTimeEnd - latencyTimeStartArray[i]);
		    ...
		    // Zeige Status
		    $("#measureState").html(
			    "<font>Latenzermittlung fehlerhaft!</font>").css(
			    "background-color", "#ff0000");
...
function extraGeoMeasure2() {
    ...
    navigator.geolocation.getCurrentPosition(extraGeoCallback,
	    extraGeoErrorCallback, options);
...
{ // Parameter gibt Datei an, Bsp. url50
	...
	if (connection == "NONE") {
	    ...
	    if (p_trackmode) {
		$("#measureState").html(
			"<font>Keine Internetverbindung?</font>").css(
			"background-color", "#ff0000");
	    } else {
		document.getElementById("stateOfSpeed").firstChild.data = "Keine Internetverbindung?";
		document.getElementById("stateOfSpeed").setAttribute("style",
			"color:red");
	    }
	    return;
	} else {
	    if (p_trackmode) {
		$("#measureState").html("<font>Messung l&auml;uft ...</font>")
			.css("background-color", "#009FE4");
	    } else {
    ...
    $
		...
		success : function(data) {
		    ...
		    if (p_trackmode) {
			$("#measureState").html(
				"<font>Messung l&auml;uft ...</font>").css(
				"background-color", "#009FE4");
		    } else {
			document.getElementById("stateOfSpeed").firstChild.data = "Messung läuft..";
			document.getElementById("stateOfSpeed").setAttribute(
				"style", "color: #000");
		    }
		...
		error : function(jqXHR, textStatus, errorThrown) {
		    ...
		    // XHR einen NETWORK_ERR zurück (code: 19 bzw. 101)
		    // (Webkit/Safari)
		    // "0x80004005 (NS_ERROR_FAILURE)" bzw. "0x80040111 (NS
		    // ERROR NOT AVAILABLE)" Firefox
		    if (errorThrown.code == 19
			    || errorThrown.code == 101
			    || (errorThrown.message.search("0x80004005") || errorThrown.message
				    .search("0x80040111")) != -1) {
			downloadTimeSum = 0;
			if (p_trackmode) {
			    $("#measureState").html(
				    "<font>Keine Internetverbindung?</font>")
				    .css("background-color", "#ff0000");
			} else {
			    document.getElementById("stateOfSpeed").firstChild.data = "Keine Internetverbindung?";
			    document.getElementById("stateOfSpeed")
				    .setAttribute("style", "color:red");
			}
		    } else {
			// Zeige Status
			$("#measureState").html(
				"<font>Download fehlerhaft!</font>").css(
				"background-color", "#ff0000");
		    }

		    log("Fehler mit dem runterlaen der datei" + jqXHR
...
// nicht direct mit XHR
{
	...
	$.ajax({
		...
		downloadTimeSum = downloadTimeSum
			+ (downloadTimeEnd - downloadTimeStartArray[i]);
	    ...
	    error : function(jqXHR, textStatus, errorThrown) {
		$("#measureState").html("<font>Download fehlerhaft!</font>")
			.css("background-color", "#ff0000");

		log("Fehler mit dem runterlaen der datei" + jqXHR + textStatus
			+ errorThrown);
		log("jqXHR: " + jqXHR);
		log("textStatus: " + textStatus);
		log("errorThrown: " + errorThrown);
...
function send(sendAsMail) {
    ...
    // zeige Status
    $("#measureState").html("<font>Messung absenden ...</font>").css(
	    "background-color", "#009FE4");
...
function sendReportAsMail() {
    ...
    var link = "mailto:speedtrack.report@doubleslash.de?"
	    + "subject="
	    + "Track und Logfile der Messung mit Webapp"
	    + "&body="
	    + "Da ein Fehler aufgetreten ist konnte der Benutzer die Messung per Email versenden."
	    + "%0A" + "Es ist Folgender Fehler aufgetreten: " + errorMessage
	    + "%0A" + textResultMail + "%0A" + "%0A" + "%0A" + logFile;
...
function isLogedIn() {
	...
	if (data.success) {
	    ...
	    $("#measureState").html("<font>Login timed out!</font>").css(
		    "background-color", "#ff0000");
...
/*
 * Funktion die das Ergebniss an das servlet überträgt, bei einem Fehler wird
 * ein Sendebutton eingeblendet mit dem man dann eine Email mit dem Track und
 * dem Log senden kann
 */
function transfer(result, name) {
    ...
    $
	    .post(
		    urlString + "speedtrack/STWizardServlet",
		    {
			trackString : result,
			trackName : name,
			action : "webapp"
			...
			if (successString) { // Fehler oder Erfolg anzeigen
			    ...
			    if (messageString != "Track benötigt mindestens "
				    + minTrackPoints + " Messpunkte") {
...
function save(downloadRate, latency, latitude, longitude, timestamp, measureTyp) {
    ...
    var textResultNew = textResult + downloadRate + "|" + latency + "|"
	    + latitude + "|" + longitude + "|" + timestamp + "|" + measureTyp
	    + "\r";
    textResult = textResultNew;

    // Ergebnisse abspeichern für Email
    textResultNew = textResultMail + downloadRate + "|" + latency + "|"
	    + latitude + "|" + longitude + "|" + timestamp + "|" + measureTyp
	    + "%0A";
