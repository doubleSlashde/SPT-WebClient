...
var originLat;
// Breitengrad des eigenen Standorts
var originLng;
...
// Debug)
var minTrackPoints;
// Mindestanzahl an Messpunkten

// Variablen fuer Latenz und Downloadraten Messung
var latencyTimeStart;
// Starteit Latenz
var latencyTimeStartArray;
// Starteitfeld Latenz
var latencyTimeEnd;
// Endzeit Latenz
var latencyTimeSum;
...
// gibt an ob Latenzzeit schon ermittelt wurde
var downloadTimeStart;
// Startzeit download
var downloadTimeStartArray;
// Startzeitfeld download
var downloadTimeEnd;
// Endzeit download
var downloadTimeSum;
// Gesamtzeit download
var finDownloadMeasure;
// gibt an ob Downloadrate schon ermittelt wurde
var xhr;
// Variable für XHR-Objekt
var shortMeasure;
// gibt an welche Messung genommen werden soll(eine alle
// 10sec oder 3 alle 30sec)
var finish;
// Zeigt an ob Messung fertig ist
var numberOfmeasurements;
// Anzahl der bisherigen Messungen

// Variablen für Zeitmessung
var aktiv;
// Variable um den Zeitlichenintervall zu steuern
var startTime;
// Startzeit
var endTime;
// Endzeit
var now;
// Variable in der jetztige Zeit abgespeichert wird
var timeStamp;
// Zeitpunkt an der die Messung Gestartet wurde.
var startTimeHelp;
// Hilfsvariable um die Zeit zu stoppen von Latenz und
// Downloadrate
var endTimeHelp;
// Hilfsvariable um die Zeit zu stoppen von Latenz und
// Downloadrate
...
// anderes
var url;
// Url der Downloaddatei
var url50;
// URL von 50 Kilobyte Downloadfile
var url100;
// URL von 100 Kilobyte Downloadfile
var url150;
// URL von 150 Kilobyte Downloadfile
var name;
// Variable für Benutzername
var isAndroidPhone;
// gibt an Ob es ein Androidhandy ist
var version;
// gibt aktuelle Version an
var urlString;
// Speichert richtige URL fuer Webapp oder Andropid

// Variablen für die Ausgabe
var textResult;
// hier wird der Ergebnisstring gespeichert der an die Homepage
// geschickt wird
var textResultMail;
// hier wird der Ergebnisstring im Emailformat gespeichert
var logIndex;
// Anzahl von Logeinträgen
var logFile;
// speichert Einträge von Log in String ab
var errorMessage;
...
function printTrack(p_dataUrlString, p_trackmode) {
    ...
    var latency = 0;
    // Variable für die Latenzzeit, wird an Upload Servlet
    // übergeben
    var downloadRate = 0;
    // Variable für die Downloadrate
    var datasize = 0;
    // Größe der Datei in kb -> Berechnung der Downloadrate
    var latencyShow = null;
    // Wert der im Frontend angezeigt wird
    var downloadRateShow = null;
    // Wert der an das Servlet übergeben wird

    if (p_dataUrlString == "url50")
        datasize = 50;
    // setze Größe durch Url
    ...
    if (downloadTimeSum != 0) {
        ...
        var time = 0;
        time = ((downloadTimeSum)) / 1000.0;
        // sec

        if (time != 0) {
            if (shortMeasure) {
                downloadRateShow = ((datasize / time));
                // Normale Messungen,
                // Angabe in KB/s
                // (Bytes/s =
                // downloadRate*1000)
                ...
                latency = ((latencyTimeSum) / 2);
                // milli sec
            } else {
                downloadRate = ((datasize / time));
                // "3er" Messungen

                latency = ((latencyTimeSum) / 3 / 2);
                // milli sec
            }
        }
    } else {
        downloadRate = 0;
        latency = 0;
        // Wert der an das Upload Serlver übergeben wird (muss
        // Zahl sein)
        latencyShow = '&#8211;';
        // Wert der im Frontend angezeigt wird
        downloadRateShow = 0;
...
// url50->speedtrack.org/.../50kb.txt
{
    ...
    // da sonst bei Threadwechsel die Daten
...
// XHR
{
    ...
    $.ajax({
        ...
        error : function(jqXHR, textStatus, errorThrown) {
            ...
            if (errorThrown.code == 19 || errorThrown.code == 101 || (errorThrown.message.search("0x80004005") || errorThrown.message.search("0x80040111")) != -1) {
                downloadTimeSum = 0;
                if (p_trackmode) {
                    $("#measureState").html("<font>Keine Internetverbindung?</font>").css("background-color", "#ff0000");
                } else {
    ...
    for (var i = 0; i < 3; i++) {
        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                endTimeHelp = new Date();
                // da sonst bei threadwechsel die
...
function sendReportAsMail() {
    ...
    var link = "mailto:speedtrack.report@doubleslash.de?" + "subject=" + "Track und Logfile der Messung mit Webapp" + "&body=" + "Da ein Fehler aufgetreten ist konnte der Benutzer die Messung per Email versenden." + "%0A" + "Es ist Folgender Fehler aufgetreten: " + errorMessage + "%0A" + textResultMail + "%0A" + "%0A" + "%0A" + logFile;
